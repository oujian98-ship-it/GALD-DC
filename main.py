...
parser.add_argument('--config', default=None, help='path to config file (autodetected if None)')
...
parser.add_argument('--beta_radius', default=0.2, type=float, help='EMA decay rate for class radii (β_radius)')
...
parser.add_argument('--lambda_cal', default=0.3, type=float, help='Tail radius calibration blending factor (0=pure prior, 1=pure observed)')

# Margin constraint parameters 
parser.add_argument('--eta_m', default=0, type=float, help='Margin loss weight')
...
parser.add_argument('--beta_cons', default=0.3, type=float, help='Consistency loss weight ')
...
parser.add_argument('--stage1_end_epoch', default=10, type=int, help='Stage 1 maximum epochs (will transition early if converged)')
parser.add_argument('--stage2_epochs', default=5, type=int, help='Stage 2 duration (number of epochs for Diffusion training)')
parser.add_argument('--stage3_epochs', default=10, type=int, help='Stage 3 duration (number of epochs for fine-tuning)')

# Dynamic Stage 1 transition
parser.add_argument('--enable_dynamic_stage1', default=True, type=bool, help='Enable early Stage 1 exit when CE loss converges')
parser.add_argument('--convergence_window', default=15, type=int,help='Window size (epochs) for convergence detection')
parser.add_argument('--convergence_threshold', default=0.005, type=float,help='CE loss convergence threshold (relative change)')
parser.add_argument('--min_stage1_epochs', default=50, type=int,help='Minimum epochs for Stage 1 before checking convergence')
...
def main():
    ...
    if args.config is None:
        if args.dataset == "CIFAR10":
            args.config = "./config/cifar10/cifar10_LSC_Mixup.txt"
        elif args.dataset == "CIFAR100":
            args.config = "./config/cifar100/cifar100_LSC_Mixup.txt"
        elif args.dataset == "ImageNet":
            args.config = "./config/ImageNet/ImageNet_LSC_Mixup.txt"
        
        if args.config:
            print(f">>> [Auto-Select] Selected config: {args.config}")
        else:
            print(f">>> [Warning] No default config found for dataset={args.dataset}")
    ...
    if args.imb_factor is None or str(args.imb_factor).lower() == 'none':
        log_id = args.dataset
    else:
        log_id = f"{args.dataset}_{args.imb_factor}"
    
    logs_dir = os.path.join("./logs", log_id)
    os.makedirs(logs_dir, exist_ok=True)
    log_filename = os.path.join(logs_dir, f"{log_id}_{current_time}.log")
    ...
    # Training process based on the training strategy (forced to GALD-DC)
    print("Using training strategy: GALD-DC")
    train_gald_dc(args)
