...
try:
    ...
    print(
        "This may be due to missing ema_pytorch dependency or other module path issues"
    )
    DDPM_AVAILABLE = False

    # Provide simple fallback implementation
    ...
    class ConditionalDiffusion1D(nn.Module):
        def __init__(self, unet, seq_length=64, timesteps=1000, objective="pred_noise"):
            super().__init__()
            self.model = unet
...
class ModelManager:
    ...
    def initialize_models(
        self, num_classes: int, dataset_info: Dict
        ...
        # 2. Select classifier type based on configuration
    ...
    def save_best_models(self, encoder, classifier, diffusion_model, accuracy, bs_acc):
        ...
        # Save best BS accuracy model
        if bs_acc > getattr(self, "_best_bs_acc", -1):
            self._best_bs_acc = bs_acc
            save_path_bs = r"./saved_models/ckpt_best_bs_strategy_A.checkpoint"
            ...
            torch.save(model_state, save_path_bs)
            print(
                f"The model with the best BS (Balanced Softmax) accuracy has been saved to: {save_path_bs}"
            )
